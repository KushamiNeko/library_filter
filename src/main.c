...
void dataReceived(GtkWidget *widget, GdkDragContext *context, gint x, gint y,
                  GtkSelectionData *data, guint info, guint time,
                  gpointer userData) {
  char *t = gtk_selection_data_get_text(data);
 ...
 *  * accept. The string type is arbitrary, and negotiated between DnD widgets
 * by
...
static GtkTargetEntry testList[] = {{"STRING", GTK_TARGET_OTHER_APP, 0}};
 ...
 *   * it needs to finish the operation by calling gtk_drag_finish, which will
 * emit
 ...
 *    * target type it wishes to ask the source for. Finally call
 * gtk_drag_get_data
 ...
 * cannot,
 *    * it should default to a "safe" type such as a string or text, even if
 * only to
 *     * print an error. Then use gtk_selection_data_set to put the source data
 * into
 *      * the allocated selection_data object, which will then be passed to the
 *       * destination. This will cause "drag-data-received" to be emitted on
 * the
 *        * destination. GdkSelectionData is based on X's selection mechanism
 * which,
 *         * via X properties, is only capable of storing data in blocks of 8,
 * 16, or
 *          * 32 bit units. */
static void drag_data_get_handl(GtkWidget *widget, GdkDragContext *context,
  ...
  switch (target_type) {
     ...
     *                                   * Pixbufs, (UTF-8) text, and URIs have
    ...
    case TARGET_STRING:
      ...
      gtk_selection_data_set(selection_data,
                             ...
                             _BYTE, (guchar *)string_data, strlen(string_data)
...
int main(int argc, char const *argv[]) {
  ...
  // *                   */
  // gtk_drag_source_set(coin_source,      /* widget will be drag-able */
  ...
  // g_signal_connect(well_dest, "drag-leave", G_CALLBACK(drag_leave_handl),
  // NULL);

  // g_signal_connect(well_dest, "drag-motion", G_CALLBACK(drag_motion_handl),
  //                 NULL);

  // g_signal_connect(well_dest, "drag-drop", G_CALLBACK(drag_drop_handl),
  // NULL);
  ...
  // g_signal_connect(coin_source, "drag-begin", G_CALLBACK(drag_begin_handl),
  //                 NULL);

  // g_signal_connect(coin_source, "drag-end", G_CALLBACK(drag_end_handl),
  // NULL);
  ...
  gtk_drag_dest_set(win, GTK_DEST_DEFAULT_ALL, testList, 1, GDK_ACTION_COPY);
  g_signal_connect(GTK_WIDGET(win), "drag-data-received",
                   G_CALLBACK(dataReceived), NULL);
